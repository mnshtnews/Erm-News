...
─────────────────────
The central orchestration service for Eram News monitor.

ArticlePipeline wires together all subsystems:
  EremNewsScraper → Classifier → Repository → Telegram
...
from src.scraper.eremnews_engine import EremNewsScraper
from src.telegram.sender import TelegramSender


class ArticlePipeline:
    """
    End-to-end article processing pipeline for Eram News.
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self._scraper = EremNewsScraper(settings)
    ...
    async def start(self) -> None:
        ...
        logger.info("Starting ArticlePipeline (Eram News) …")
    ...
    async def run_forever(self) -> None:
        ...
        while self._running:
            ...
            logger.debug(
                f"Poll cycle complete — sleeping {self._settings.poll_interval_seconds}s"
            )
