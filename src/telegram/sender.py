...
───────────────────────
Telegram notification module for Eram News monitor.
Professional news card format — clean HTML, no emojis.
"""
...
_CATEGORY_MAP = {
    NewsClassification.UAE:          "أخبار رياضية إماراتية",
    NewsClassification.ARAB:         "أخبار رياضية عربية",
    NewsClassification.GLOBAL:       "أخبار رياضية عالمية",
    NewsClassification.UNCLASSIFIED: "أخبار رياضية",
}

_SOURCE_LABEL = "إرم نيوز"
...
class TelegramSender:
    ...
    async def start(self) -> None:
        ...
        logger.info("Telegram bot initialised (Eram News)")
    ...
    def _build_caption(self, article: Article, limit: int = 950) -> str:
        category  = _CATEGORY_MAP.get(article.classification, "أخبار رياضية")
        ...
        lines.append(f"<b>{self._escape(_SOURCE_LABEL)} | {self._escape(category)}</b>")
        lines.append("")
